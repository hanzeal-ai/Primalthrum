...
export const DEFAULT_ABUSE_POLICIES: readonly AbusePolicy[] = [
  ...
  policy('privacy_export', 'privacy_export', 'POST', /^\/api\/settings\/privacy\/export$/, [user(5, HOUR)]),
  policy('account_deletion', 'account_deletion', 'POST', /^\/api\/settings\/privacy\/deletion$/, [user(5, DAY)]),
  policy('account_deletion_cancel', 'account_deletion_cancel', 'DELETE', /^\/api\/settings\/privacy\/deletion$/, [user(5, DAY)]),
