...
test('@desktop customer completes trial onboarding and uses a RAG Agent', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: '消息', exact: true })
    .fill('What is the Project Aster launch date?')
  await page.getByRole('button', { name: '发送' }).click()
  await expect(page.getByText('mock response: What is the Project Aster launch date?').last())
    .toBeVisible({ timeout: 30_000 })
  await page.getByText(/运行活动/).click()
  await expect(page.getByText('Tool file_reader is ready')).toBeVisible()
  await expect(page.getByText('Applied skill research')).toBeVisible()
  await expect(page.getByText('Loaded 1 memory summaries')).toBeVisible()
  await expect(page.getByText('Cache hit for intake')).toBeVisible()
  await expect(page.getByText('Saved run summary to memory')).toBeVisible()
  ...
  await expect(page.getByText('launch.md', { exact: true }).last()).toBeVisible()
