...
| Text, voice, attachments, Tool, Skill, Memory, Cache, and optional RAG | Browser tests cover text, browser speech input/playback, permission denial, file upload, SQLite RAG, and source rendering. Agent and Server suites cover Tool, Skill, Memory, and Cache runtime contracts. | Partial | Add direct hosted-surface evidence for Tool, Skill, Memory, and Cache behavior; complete a physical microphone STT/TTS round trip. |
...
The next code change is limited to direct Hosted Agent evidence for Tool, Skill,
Memory, and Cache behavior. Do not mark P23 or P24 `Done` until every Partial row
above is supported by current evidence.
